...
// Coordinates in us-atlas 960x600 Albers USA space
// Austin intentionally offset slightly south of NOLA for visual clarity
const CITY_COORDS = {
  'New Orleans':    [720, 441],
  'Baton Rouge':    [701, 436],
  'Tampa':          [790, 468],
  ...
  'Nashville':      [730, 360],
  'Austin':         [554, 462],
  'Salt Lake City': [298, 298],
};

// us-atlas topojson is pre-projected to 960x600 — delta-decode only, no re-projection
...
function Sel({ value, onChange, options }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)} style={{
      ...
      appearance: 'none', color: '#1a1a1a', minWidth: 128,
...
function DirectoryTab({ funds, loading, error, search, setSearch, cityFilter, setCityFilter,
  ...
  sizeFilter, setSizeFilter, cities, types, stages, sectors, selected, setSelected, onClearAll, hasActiveFilters }) {
  return (
    <div>
...
function NetworkTab({ funds, mapPaths, mapLoaded }) {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 110px)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, padding: '16px 20px 12px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        {/* SVG map — full 960x600 us-atlas coordinate space */}
        <div style={{ flex: 1, minHeight: 0, border: '1px solid #ddd8d0', overflow: 'hidden', position: 'relative' }}>
          <svg
            viewBox="0 0 960 600"
          ...
          >
            ...
            {mapPaths.length > 0 ? (
              ...
              /* Simplified fallback outline of continental US */
              <g>
                <rect x="70" y="60" width="700" height="420" rx="8" fill="#e8e1d5" stroke="#c4bdb0" strokeWidth="1" />
                {/* Alaska box */}
                <rect x="70" y="440" width="160" height="120" rx="4" fill="#e8e1d5" stroke="#c4bdb0" strokeWidth="1" />
                {/* Hawaii box */}
                <rect x="240" y="500" width="120" height="60" rx="4" fill="#e8e1d5" stroke="#c4bdb0" strokeWidth="1" />
                <text x="480" y="270" textAnchor="middle" fontSize="13" fill="#c4bdb0" fontFamily='"DM Mono", monospace' letterSpacing="2">
                  LOADING MAP…
                </text>
              </g>
            )}
              ...
              return (
                <g key={city} transform={`translate(${cx},${cy})`} style={{ cursor: 'pointer' }}
                  ...
                  <text textAnchor="middle" dy="0.35em"
                    ...
                    fill="#faf6f0" style={{ pointerEvents: 'none' }}>
                    {count}
                  </text>
                  ...
                  {hov && (
                    <g transform={`translate(0,${-r - 22})`} style={{ pointerEvents: 'none' }}>
                      <rect x={-70} y={-10} width={140} height={20} rx="2" fill="#1a1a1a" />
          ...
          <>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #d4cfc7', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <button onClick={() => setMapCity(null)} style={{ background: 'none', border: '1px solid #bbb',
                cursor: 'pointer', width: 26, height: 26, fontSize: 14,
...
export default function App() {
  ...
  useEffect(() => {
    fetch('https://cdn.jsdelivr.net/npm/us-atlas@3/states-10m.json')
      .then(r => r.json())
      .then(t => { setMapPaths(decodeTopojson(t)); setMapLoaded(true); })
      .catch(e => { console.error('Map fetch failed:', e); setMapLoaded(false); });
  }, []);
  ...
  const hasActiveFilters = search || cityFilter !== 'All Cities' || typeFilter !== 'All Types' ||
    stageFilter !== 'All Stages' || sizeFilter !== 'All Sizes' || sectorFilter !== 'All Sectors';

  const clearAll = () => {
    setSearch(''); setCityFilter('All Cities'); setTypeFilter('All Types');
    setStageFilter('All Stages'); setSizeFilter('All Sizes'); setSectorFilter('All Sectors');
  };
    ...
    <div style={{ minHeight: '100vh', background: '#faf6f0', fontFamily: '"DM Mono", monospace', color: '#1a1a1a' }}>
      ...
      <header style={{ borderBottom: '2px solid #1a1a1a', background: '#faf6f0', display: 'flex', alignItems: 'stretch', minHeight: 72 }}>
        ...
        <div style={{ padding: '0 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center',
          ...
          onClick={() => { setTab('directory'); clearAll(); }}>
      ...
      {tab === 'directory'
        ...
        : <NetworkTab funds={funds} mapPaths={mapPaths} mapLoaded={mapLoaded} />
      }
    </div>
  );
}
